...
├─ web-admin/                Vue3 管理后台前端项目
├─ docs/                     项目设计、接口对接等补充文档
├─ deploy/                   部署脚本和数据库初始化脚本
└─ README.md                 项目说明文档
```

## 配置文件位置

后端配置由 `backend/src/t_alpha/config.py` 中的 `Settings` 类统一读取，实际运行时会优先读取 `backend/.env` 文件。
...
然后打开 `backend/.env`，填写自己的配置。常用配置如下：
...
开发时推荐使用“可编辑安装”，这样你修改 `backend/src/` 里的代码后，不需要重新安装包：
...
- `t_alpha.main:app` 表示启动 `backend/src/t_alpha/main.py` 里的 FastAPI 应用。
...
这个命令会读取 `backend/.env` 中的 `APP_HOST` 和 `APP_PORT`。如果没有修改配置，默认会在 `127.0.0.1:8867` 启动。
...
4. `.env` 是否在 `backend/` 目录，而不是放到了 `backend/src/` 或其他目录。

### 配置改了为什么没生效？

如果正在使用 `uvicorn`，请停止服务后重新启动。`backend/.env` 是程序启动时读取的，服务运行中修改 `.env` 通常不会自动生效。

## 安全提醒

`backend/.env` 里会保存 AmazingData、MySQL、SMTP 等账号密码。请只提交 `backend/.env.example`，不要提交真实的 `.env` 文件。
